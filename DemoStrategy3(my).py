    ...
    def exec_signal(self,tick1,tick2):
        if self.buySig:
            ...
            if tick2.lastPrice>0:
                ...
                self.cover(tick2.upperLimit, self.pos.get(self.symbolList[1]), symbol=self.symbolList[1])
            if tick1.lastPrice>0:
                self.sell(tick1.upperLimit, self.pos.get(self.symbolList[0]), symbol=self.symbolList[0])
            self.has_canceled = False
